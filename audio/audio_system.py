...
class SystemAudioListener(QThread):
    ...
    def force_stop(self):
        """Принудительно завершить захват и отправить накопленное."""
        if self._active:
            self._active = False
            self._flush()
    ...
    def _flush(self):
        if self._buffer:
            ...
            self._buffer = []
            print(f"[sysaudio] flush: {combined!r}")
            self.transcribed.emit(combined)
            ...
            while self._running and stream.is_active():
                ...
                if time.time() - self._capture_start > _MAX_CAPTURE_S:
                    ...
                    if is_speaking and speech_buffer:
                        ...
                        self._transcribe_to_buffer(audio)
                        speech_buffer       = []
                        is_speaking         = False
                        speech_frame_count  = 0
                        silence_frame_count = 0
    ...
    def _transcribe_to_buffer(self, audio: np.ndarray):
        try:
            ...
            if text:
                print(f"[sysaudio] fragment: {text!r}")
